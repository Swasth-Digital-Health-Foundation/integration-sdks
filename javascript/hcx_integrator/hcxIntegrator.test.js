// filename: hcxIntegrator.test.js

import { HCXIntegrator } from "./hcx_integrator.js";
import HcxOperations from "./utils/hcx_operations.js";
import axios from "axios"; // Assuming axios or a similar library is being used inside HCXIntegrator

jest.mock("axios");

describe("HCXIntegrator Tests", () => {
  let config, fhirPayload, hcxIntegrator;
  ...
  beforeEach(() => {
    ...
    hcxIntegrator = new HCXIntegrator(config);
    axios.post.mockResolvedValue({ data: "mockResponse" });

    test("processOutgoing should return expected response", async () => {
      ...
      expect(responseOutgoing).toEqual(expect.anything());
    });

    test("processIncoming should process the payload and return expected response", async () => {
      const sub = HcxOperations.CLAIM_SUBMIT;
      ...
      expect(responseIncoming).toEqual(expect.anything());
    });
  });
});
