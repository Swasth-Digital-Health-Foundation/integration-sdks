import { HCXIntegrator } from "../src/hcx_integrator";
...
jest.mock("../src/impl/HCXOutgoingRequest.js");
jest.mock("../src/impl/HCXIncomingRequest.js");
...
describe("HCXIntegrator Integration Tests", () => {
  ...
  beforeEach(() => {
    HCXOutgoingRequest.mockClear();
    HCXIncomingRequest.mockClear();
  });

  it("should process outgoing request", async () => {
    const hcxIntegrator = new HCXIntegrator(mockConfig);

    const mockProcess = jest.fn();
    HCXOutgoingRequest.mockImplementation(() => {
      return { process: mockProcess };
    });
    mockProcess.mockResolvedValue("mockResponse");

    const response = await hcxIntegrator.processOutgoingRequest(
      "fhirPayload",
      "recipientCode",
      "operation",
      "apiCallId",
  ...
  it("should process outgoing callback", async () => {
    const hcxIntegrator = new HCXIntegrator(mockConfig2);
    ...
    const response = await hcxIntegrator.processOutgoingCallback(
      "fhirPayload",
      "recipientCode",
      "operation",
      "actionJwe",
  ...
  it("should process incoming request", async () => {
    ...
    const mockProcess = jest.fn();
    HCXIncomingRequest.mockImplementation(() => {
      return { process: mockProcess };
    });
    mockProcess.mockResolvedValue("mockResponse");

    const response = await hcxIntegrator.processIncoming(
      "encryptedPayload",
      "operation",
      "apiCallId",
      "correlationId",
