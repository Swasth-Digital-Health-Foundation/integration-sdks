...
namespace Io.HcxProtocol.Dto
{
    ...
    public class BaseRequest : IBaseRequest, IDisposable
    {
        ...
        public bool ValidateHeadersData(List<string> mandatoryHeaders, Operations operation, Dictionary<string, object> error)
        {
            ...
            if (ValidateCondition(GetApiCallId().GetType() != typeof(string), error, ErrorCodes.ERR_INVALID_API_CALL_ID.ToString(), ResponseMessage.INVALID_API_CALL_ID_ERR_MSG))
                return true;
            if (ValidateCondition(GetCorrelationId().GetType() != typeof(string), error, ErrorCodes.ERR_INVALID_CORRELATION_ID.ToString(), ResponseMessage.INVALID_CORRELATION_ID_ERR_MSG))
                return true;
            if (ValidateCondition(!GetTimestamp().IsValidTimestamp(), error, ErrorCodes.ERR_INVALID_TIMESTAMP.ToString(), ResponseMessage.INVALID_TIMESTAMP_ERR_MSG))
                return true;
            if (ValidateCondition(ProtocolHeaders.ContainsKey(Constants.WORKFLOW_ID) && GetWorkflowId().GetType() != typeof(string), error, ErrorCodes.ERR_INVALID_WORKFLOW_ID.ToString(), ResponseMessage.INVALID_WORKFLOW_ID_ERR_MSG))
                return true;
        ...
        public bool ValidateJwePayload(Dictionary<string, object> error, string[] payloadArr)
        {
            ...
            {
                error.Add("Exception while validating jwe payload", ex.Message.ToString());
                _logger.Error("Exception while validating jwe payload", ex.Message.ToString());
