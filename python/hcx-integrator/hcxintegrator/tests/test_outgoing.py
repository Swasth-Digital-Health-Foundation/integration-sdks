...
try:
    with open(os.path.join(curdir, "response_outgoing.json"), "r") as f:
        pretest = json.load(f)
    if compare_output(response, pretest):
        print(f'Test outgoing successful!')
except Exception as e:
    print(f'Test failed! Following exception occured: {e}')
    

# Action JWE outgoing test
with open(os.path.join(curdir, "response_outgoing.json"), "r") as f:
    actionJwe = json.loads(f.read())["payload"]
response = hcxIntegrator.processOutgoing(fhirPayload, None, HcxOperations.CLAIM_ON_SUBMIT, actionJWE=actionJwe)
print(response)
