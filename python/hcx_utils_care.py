...
class HcxUtils:
    ...
    def __init__(self, protocolBasePath, participantCode,authBasePath,username,password,encryptionPrivateKeyURL,igUrl):
    ...
    def encryptJWE(self, recipientCode = None, fhirPayload = None):
        ...
        regsitry_data = self.searchRegistry(searchField = "participant_code", searchValue = recipientCode)        
    ...
    def decryptJWE(self, encryptedString):     
        ...
        return {"headers": dict(json.loads(jwetoken.payload.decode("utf-8"))), "payload":dict(json.loads(jwetoken.payload.decode('utf-8')))}
    ...
    def generateOutgoingHcxCall(self, fhirPayload, operation ,recipientCode):
        encryptedString = self.encryptJWE(recipientCode=recipientCode, fhirPayload=fhirPayload)
        response =  self.makeHcxApiCall(operation=operation, encryptedString=encryptedString)
